...
{
	dev_data->poll_interval = 500;
	dev_data->min_interval = 1;
...
{
	sysfs_remove_group(&acc->input_dev->dev.kobj, &lis3de_attribute_group);
	cancel_delayed_work_sync(&acc->input_work);
		...
		const struct i2c_device_id *id)
{
	...
	*/
	...
	err = sysfs_create_group(&acc->input_dev->dev.kobj, &lis3de_attribute_group);
	if (err < 0) {
		printk(KERN_ERR
...
static int __init lis3dh_acc_init(void)
{
	...
	if (input_fetch_sysconfig_para(&(gsensor_info.input_type))) {
		...
		if (0 != ret){
			printk("%s:gsensor_ops.init_platform_resource err. \n", __func__);
		}
	}
