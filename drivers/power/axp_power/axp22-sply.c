...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	charger->rest_vol	= (int)	(val & 0x7F);
	//current correction
	axp_read(charger->master,AXP22_DATA_BUFFER8,&delta);
	axp_reads(charger->master,0xe4,2,ocv_colum_percent);
	if(axp_debug){
	printk("charger->ext_valid:%d,charger -> bat_current_direction:%d, ocv_colum_percent[0]:%d, ocv_colum_percent[1]:%d, charger->is_on:%d, charger->rest_vol:%d, pre_rest_vol:%d\n",
	       charger->ext_valid, charger -> bat_current_direction,ocv_colum_percent[0], ocv_colum_percent[1], charger->is_on, charger->rest_vol, pre_rest_vol);
	}
	if(charger->ext_valid ==1 && charger -> bat_current_direction == 1 && (ocv_colum_percent[0]&0x7f) >95 && charger->is_on == 0 && charger->rest_vol < 100 )
	{
	  if(delta + charger->rest_vol < 100)
	  {
		delta++;
		if(axp_debug){
			printk("delta+: charger->rest_vol:%d,delta:%d\n", charger->rest_vol,delta);
		}
	}
	}
	else if(charger->ext_valid == 0)
	{

	  if(delta >0 && charger->rest_vol + delta >= pre_rest_vol && charger->rest_vol + delta != 100)
	  {
		delta--;
          }
	  else if(delta > 0 && charger->rest_vol < 30)
	  {
		  delta--;
       ...
       axp_write(charger->master,AXP22_DATA_BUFFER8,delta);
	if(axp_debug){
		printk("charger->rest_vol:%d, delta:%d, pre_rest_vol:%d \n", charger->rest_vol, delta, pre_rest_vol);
	}
	if(charger->rest_vol + delta > 100)
	  {
	     charger->rest_vol = 100;
	  }
       else
...
//current correction
	axp_read(charger->master,AXP22_DATA_BUFFER8,&delta);
	temp_rest_vol = (int) (val2 & 0x7F)+delta;
	if(temp_rest_vol < 100){
		charger->rest_vol      = temp_rest_vol;
	}else{
		charger->rest_vol      = 100;
	}
...
static int axp22_resume(struct platform_device *dev)
{
	...
	if(charger->ext_valid ==1 && charger -> bat_current_direction == 1 && (ocv_colum_percent[0]&0x7f) >95 && charger->is_on == 0 && charger->rest_vol < 100 )
	{
	  if(delta + charger->rest_vol < 100)
	  {
		...
		if(axp_debug)
		{
			printk("delat+: charger->rest_vol:%d,delta:%d\n", charger->rest_vol,delta);
		}
	  }
	}
	else if(charger->ext_valid == 0)
